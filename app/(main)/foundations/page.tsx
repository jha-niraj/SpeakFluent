...
interface ModuleContent {
    sections?: Array<{ title: string; [key: string]: unknown }>;
        ...
        const fetchFoundationData = async () => {
            if (session?.user?.id) {
                try {
                    ...
                    if (result.success) {
                        if (result.data) {
                            setFoundationData(result.data as FoundationData);
                            setModules((result?.data?.modules || []).map(module => ({
                                ...module,
                        ...
                        if (result?.data?.modules.length === 0 && result?.data?.user?.selectedLanguage) {
                            ...
                            if (updatedResult.success && updatedResult.data) {
                                setFoundationData(updatedResult.data as FoundationData);
                                setModules((updatedResult?.data?.modules || []).map(module => ({
                                    ...module,
                                    content: module.content as ModuleContent | null
                                })));
                                            ...
                                            <CardContent className="pt-0">
                                                <div className="grid grid-cols-3 gap-4 mb-6">
                                                    <div className="text-center">
                                                        <div className={`text-2xl font-bold ${isLocked ? 'text-gray-400' : 'text-teal-600'}`}>
                                                            {(module.content as ModuleContent)?.totalDuration || 45}
                                                        </div>
                                                ...
                                                <div className="space-y-2 mb-6">
                                                    ...
                                                    <div className="space-y-1">
                                                        {
                                                            (module.content as ModuleContent)?.sections?.slice(0, 3).map((section, idx: number) => (
